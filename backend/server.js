...
app.post('/api/registro', (req, res) => { // Ruta Post para registrar usuario
  const { nombre, email, password } = req.body; // Recibe nombre, email y contraseña

  if (!nombre || !email || !password) { // Verifica que no falte ningún dato
  ...
  db.query(verificar, [email], (err, results) => {
    ...
    const sql = 'INSERT INTO usuarios (nombre, email, password) VALUES (?, ?, ?)'; // Inserta el nuevo usuario
